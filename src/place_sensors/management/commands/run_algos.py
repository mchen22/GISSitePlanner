...
from django.contrib.gis.geos import LinearRing, LineString, Polygon, MultiPolygon, MultiLineString, MultiPolygon
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for site in Locations.objects.all():
            ...
            sps = PlaceSensor(xy, interval_length = s[0].rng, FOV = s[0].fov, add_final_point = False, add_original_points = False, skip_small = True)

            ct = CoordTransform(utmProj, wgsProj)
            sps.transform(ct)

            p = SensorPlacements(site=site, sensor=s[0], placement=sps)
            p.save()
